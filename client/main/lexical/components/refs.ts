...
export class ReferenceNumbers extends HTMLElement {
    ...
    constructor() {
        ...
        this._root.innerHTML = `<style>${ css }</style><div class="body"></div>`;
    ...
    update() {
        ...
        for (let ref of this._refs) {
            ...
            this._body.insertAdjacentHTML('beforeend', `<span class="num" aria-label="Reference ${number}">[${ number }]</span>`);
        }
    }
}
