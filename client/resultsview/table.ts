...
import { h, htmlTrusted, rich, richParagraphs, richTableOptions, setRich } from '../common/htmlelementcreator';
...
const trustedContent = function(html: string): ChildNode[] {
    ...
    recordTrustedContentUsage(wrapper);
    return Array.from(wrapper.childNodes);
};

const getTrustedContentUsage = function() {
    const win = window as any;
    if (win.__jamoviTrustedContentUsage === undefined)
        win.__jamoviTrustedContentUsage = { tags: {}, attributes: {}, signatures: {} };
    return win.__jamoviTrustedContentUsage;
};

const publishTrustedContentUsage = function() {
    const usage = getTrustedContentUsage();

    if (window.parent === window)
        return;

    window.parent.postMessage({
        type: 'jamovi:trustedContentUsage',
        usage: {
            tags: { ...usage.tags },
...
getTrustedContentUsage();
publishTrustedContentUsage();

const recordTrustedContentUsage = function(wrapper: Element) {
    const usage = getTrustedContentUsage();
    ...
    wrapper.querySelectorAll('*').forEach(element => {
        ...
        for (let attribute of Array.from(element.attributes)) {
            ...
            usage.attributes[key] = (usage.attributes[key] ?? 0) + 1;
            signatureParts.push(`[${name}]`);
        }

        const signature = signatureParts.join('');
        if (usage.signatures[signature] === undefined)
            changed = true;
        usage.signatures[signature] = (usage.signatures[signature] ?? 0) + 1;
    });

    if (changed)
        publishTrustedContentUsage();

    if (changed && window.localStorage.getItem('jamovi:logTrustedContent') === '1')
        console.debug('trustedContent usage', usage);
};
...
const appendTableRichContent = function(element: Element, content: string) {
    if (content === '')
        ...
        element.append(rich(content, richTableOptions));
};
...
export class View extends Elem.View<Model> {
    ...
    render() {
        ...
        if (this.model.attributes.title)
            setRich(this.$titleText, this.model.attributes.title, richTableOptions);
                ...
                if ( ! cell || ! cell.combineBelow || (cell && cell.value !== '')) { //skip cells that are blank
                    ...
                    if (cell) {
                        ...
                        if (content === '&nbsp;')
                            ...
                            tableCell.append(rich(String(content), richTableOptions));
