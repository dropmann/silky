...
export class LayoutSupplierView<P extends SupplierViewProperties<U>, U = InferType<P>> extends ControlContainer<P, typeof SupplierLayoutGrid> implements IDragDropTarget<U> {
    ...
    onContainerRendering(context) {
        ...
        if (label !== null) {
            ...
            this.baseLayout.addCell(0, nextRow++, h('div', { id: labelId, style: 'white-space: nowrap;', class: 'silky-options-supplier-group-header' }, label));
        }
    ...
    render_variable(item: IItem<U>, row: number) {
        ...
        $item.append(h('div', { style: 'white-space: nowrap;  display: inline-block;', class: 'silky-list-item-value' }, item.value.toString()));
