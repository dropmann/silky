...
export class ContentSelector extends OptionControl<ContentSelectorProperties> {
    ...
    createItem() {
        ...
        this.header = h('div',  { class: `selector-body silky-control-margin-${this.getPropertyValue("margin")}` }, 
                            h('label', { id: this.labelId }, groupText)
                        );

        this.tablist = null;
        if (form === 'listbox') {
            this.tablist = h('select', attrs({ "aria-labeledby": this.labelId, name: name }));
    ...
    populateTabs() {
        if (this.tablist) {
            ...
            for (let i = 0; i < options.length; i++) {
                ...
                switch (form) {
                    case 'tabs':
                        let button = h('button', { class: "clickable", value: options[i].name }, optionTitle);
                        this.tablist.append(button);
                        break;
                    case 'radio':
                        let labelId = interactionManager.nextAriaId('label');
                        let label = h('label', { id: labelId }, optionTitle);
