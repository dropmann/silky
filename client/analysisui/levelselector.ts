...
export class LevelSelector extends OptionControl<LevelSelectorProperties> {
    ...
    override onRenderToGrid(grid: LayoutGrid, row, column, owner) {
        ...
        if (label !== '') {
            this.label = h('label', { for: id, class: `silky-option-combo-label silky-control-margin-${this.getPropertyValue('margin')}`, style: 'display: inline; white-space: nowrap;' }, label);
