...
import { isOptionListControl } from './optionlistcontrol';
...
export class GridTargetContainer extends GridControl {
    ...
    findTargetListControl(container) {
        if (isOptionListControl(container)) {
    ...
    getInteractions(values, minLength, maxLength) {
    ...
    getSupplierItems(action, silent) {
    ...
    findListWithSpace(fromList, format) {
        let foundList = false;
        for (let i = 0; i < this.targetGrids.length; i++) {
    ...
    _enableButtons(toolbar, value, disableSupplyOnly) {
    ...
    checkEnableState(button, disableSupplyOnly) {
    ...
    pushRowsBackToSupplier(list, rowIndex, count) {
