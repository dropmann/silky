...
export function createOptionListControl<T extends Constructor<OptionListControl>>(params, TBase: T) {
    checkParams(params);
    if (params && (params.selectable || params.selectable === undefined))
        return SelectableLayoutGrid(TBase);  
    else {
...
export function isOptionListControl(ctrl): boolean {
    return ctrl instanceof OptionListControl;
}

export class OptionListControl extends LayoutGridClasse(OptionControl(TitledGridControl)) {
    
    static create(params) {
        let classes = createOptionListControl(params, OptionListControl);
    ...
    findEmptyProperty(item, format, value) {
